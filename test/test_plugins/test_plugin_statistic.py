...
class TestStatisticsAdapterGetReport:
    ...
    def test_init_get_report(self):
        ...
        # Check Values
        assert report['total_files'] == 5 , f"Total files should be 5, got {report['total_files']}"
        assert report['successful_reads'] == 5 , f"Successful reads should be 5, got {report['successful_reads']}"
        assert report['failed_reads'] == 0 , f"Failed reads should be 0, got {report['failed_reads']}"
        assert report['success_rate'] == pytest.approx(100.0) , f"Success rate should be 100.0, got {report['success_rate']}"
        assert report['failed_rate'] == pytest.approx(0.0) , f"Failed rate should be 0.0, got {report['failed_rate']}"
        assert report['failed_files'] == [] , f"Failed files should be empty, got {report['failed_files']}"

        for file in report['successful_files']:
            assert isinstance(file, str), f"Successful file {file} should be a string, got {type(file).__name__}"
            assert file in data_str_list, f"Successful file {file} should be in the original data, got {data_str_list}"
        ...
        assert successful_reads == 5, f"Successful reads should be 5, got {successful_reads}"
        assert failed_reads == 0, f"Failed reads should be 0, got {failed_reads}"
        assert success_rate == pytest.approx(100.0), f"Success rate should be 100.0, got {success_rate}"
        assert failed_rate == pytest.approx(0.0) , f"Failed rate should be 0.0, got {failed_rate}"
    ...
    def test_init_get_report_with_exceptions(self):
        ...
        assert report['total_files'] == total_files , f"Total files should be {total_files}, got {report['total_files']}"
        assert report['successful_reads'] == successful_reads , f"Successful reads should be {successful_reads}, got {report['successful_reads']}"
        assert report['failed_reads'] == failed_reads , f"Failed reads should be {failed_reads}, got {report['failed_reads']}"
        assert report['success_rate'] == pytest.approx(success_rate) , f"Success rate should be {success_rate}, got {report['success_rate']}"
        assert report['failed_rate'] == pytest.approx(failed_rate) , f"Failed rate should be {failed_rate}, got {report['failed_rate']}"
        assert len(report['failed_files']) == failed_reads , f"Failed files should have {failed_reads} entries, got {len(report['failed_files'])}"
        assert len(report['successful_files']) == successful_reads , f"Successful files should have {successful_reads} entries, got {len(report['successful_files'])}"
        ...
        for file in report['failed_files']:
            assert isinstance(file, str), f"Failed file {file} should be a string, got {type(file).__name__}"
            assert file in data_str_list, f"Failed file {file} should be in the original data, got {data_str_list}"
        ...
        # Check if the getters return the expected values
        assert get_total_files == total_files , f"Total files should be {total_files}, got {get_total_files}"
        assert get_successful_reads == successful_reads, f"Successful reads should be {successful_reads}, got {get_successful_reads}"
        assert get_failed_reads == failed_reads, f"Failed reads should be {failed_reads}, got {get_failed_reads}"
        assert get_success_rate == pytest.approx(success_rate), f"Success rate should be {success_rate}, got {get_success_rate}"
        assert get_failed_rate == pytest.approx(failed_rate) , f"Failed rate should be {failed_rate}, got {get_failed_rate}"
        assert len(get_failed_files) == failed_reads , f"Failed files should have {failed_reads} entries, got {len(get_failed_files)}"
        assert len(get_successful_files) == successful_reads , f"Successful files should have {successful_reads} entries, got {len(get_successful_files)}"
...
class TestStatisticsAdapterGetExceptions:
    ...
    def test_init_get_exceptions(self):
        ...
        assert isinstance(exception_dict, dict), f"Exception dict should be a dictionary, got {type(exception_dict).__name__}"
        assert len(exception_dict) == 2, f"Exception dict should have 2 entries, got {len(exception_dict)}"
        for key, value in exception_dict.items():
            assert isinstance(key, Path), f"Exception dict key should be a Path, got {type(key).__name__}"
            assert isinstance(value, Exception), f"Exception dict value should be an Exception, got {type(value).__name__}"
            assert isinstance(value, ValueError), f"Exception dict value should be a ValueError, got {type(value).__name__}"
        ...
        assert isinstance(exception_list, list), f"Exception list should be a list, got {type(exception_list).__name__}"
        assert len(exception_list) == 2, f"Exception list should have 2 entries, got {len(exception_list)}"
        for exception in exception_list:
            assert isinstance(exception, Exception), f"Exception list item should be an Exception, got {type(exception).__name__}"
            assert isinstance(exception, ValueError), f"Exception list item should be a ValueError, got {type(exception).__name__}"
...
class TestStatisticsAdapterSizeMethods:
    ...
    def test_get_total_size(self, tmp_path):
        ...
        data: Dict[Path, str | Exception] = {file1: "Content of file 1", file2: "Content of file 2", file3: "Content of file 3"}
        ...
        # Assertions
        assert isinstance(total_size, int), f"Total size should be an integer, got {type(total_size).__name__}"
        assert total_size > 0, f"Total size should be greater than 0, got {total_size}"
        assert total_size == (len(string_max) + len(string_min) + len(string_medium)), \
            f"Total size should be {len(string_max) + len(string_min) + len(string_medium)}, got {total_size}"
        
        assert isinstance(largest_file_size, tuple), f"Largest file size should be a tuple, got {type(largest_file_size).__name__}"
        assert len(largest_file_size) == 2, f"Largest file size tuple should have 2 elements, got {len(largest_file_size)}"
        assert isinstance(largest_file_size[0], Path), f"Largest file size first element should be a Path, got {type(largest_file_size[0]).__name__}"
        assert isinstance(largest_file_size[1], int), f"Largest file size second element should be an integer, got {type(largest_file_size[1]).__name__}"
        assert largest_file_size[1] == len(string_max), f"Largest file size should be {len(string_max)}, got {largest_file_size[1]}"
        assert largest_file_size[0] == file1, f"Largest file should be {file1}, got {largest_file_size[0]}"

        assert isinstance(smallest_file_size, tuple), f"Smallest file size should be a tuple, got {type(smallest_file_size).__name__}"
        assert len(smallest_file_size) == 2, f"Smallest file size tuple should have 2 elements, got {len(smallest_file_size)}"
        assert isinstance(smallest_file_size[0], Path), f"Smallest file size first element should be a Path, got {type(smallest_file_size[0]).__name__}"
        assert isinstance(smallest_file_size[1], int), f"Smallest file size second element should be an integer, got {type(smallest_file_size[1]).__name__}"
        assert smallest_file_size[1] == len(string_min), f"Smallest file size should be {len(string_min)}, got {smallest_file_size[1]}"
        assert smallest_file_size[0] == file2, f"Smallest file should be {file2}, got {smallest_file_size[0]}"

        assert isinstance(average_file_size, float), f"Average file size should be a float, got {type(average_file_size).__name__}"
        assert average_file_size > 0, f"Average file size should be greater than 0, got {average_file_size}"
        expected_average = total_size / len(data)
        assert average_file_size == pytest.approx(expected_average), \
...
class TestStatisticsAdapterCache:
    ...
    def test_cache(self, num_files: int):
        ...
        # Check if the getters return the expected values
        assert total_files == num_files, f"Total files should be {num_files}, got {total_files}"
        assert successful_reads == num_files, f"Successful reads should be {num_files}, got {successful_reads}"
        assert failed_reads == 0, f"Failed reads should be 0, got {failed_reads}"
        assert success_rate == pytest.approx(100.0), f"Success rate should be 100.0, got {success_rate}"
        assert failed_rate == pytest.approx(0.0), f"Failed rate should be 0.0, got {failed_rate}"
        assert failed_files == [], f"Failed files should be empty, got {failed_files}"
